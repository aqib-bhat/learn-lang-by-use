document.getElementById('getUsage').addEventListener('click', async () => {
  ...
  my_session = await ai.languageModel.create({ systemPrompt: "I want to learn new words in the English language. For each word I submit, give me examples of using that word in a sentence." });
...
document.getElementById('checkUsageButton').addEventListener('click', async () => {
  ...
  my_session = await ai.languageModel.create({ systemPrompt: "Tell me if I am using the word in quotes correctly." });
  let response = await my_session.prompt("Tell me if I am using the word '" + inputWord + "' correctly:" + inputSentence);
